...
test('demo index exposes fifteen live demos with distinct new sectors', () => {
  ...
  for (const slug of newDemoSlugs) {
    assert.match(data, new RegExp(`href:\\s*'/client/${slug}'`));
    assert.match(seed, new RegExp(`slug:\\s*"${slug}"`));
    assert.match(seed, new RegExp(`/demo/${slug}\\.svg`));
  }
});
